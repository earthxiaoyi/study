...
public class QuickSort {
	 ...
	 * @param right
	 */
	public void quickSort(Integer[] as,int left , int right){
		//退出条件
	 ...
	 * @return
	 */
	private int partition(Integer[] as,int left,int right,int mv){
		//1.定义两个指示位置的索引
		//左边索引位置
		int leftIndex = left;
		//右边索引位置
	 ...
	 * @return
	 */
	public int medianValue(Integer[] as,int left,int right){
	...
	//交换
	public void swap(Integer[] as,int left,int right){
		int index = as[left];
		as[left] = as[right];
		as[right] = index;
	}
	//手工排序
	public void manualSort(Integer[] as, int left, int right){
		int size = right-left+1;
		if(size == 1){
		}else if(size == 2){
			if(as[left] > as[right]){
